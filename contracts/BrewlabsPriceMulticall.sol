...
contract BrewlabsPriceMulticall {
    function getUint(address addr, bytes memory data) internal view returns (uint256 result) {
        result = 0;
    ...
    function getLpInfo(address[][] calldata pools) external view returns (uint256[] memory) {
        uint256[] memory results = new uint[](pools.length * 3);
        uint256 idx = 0;
