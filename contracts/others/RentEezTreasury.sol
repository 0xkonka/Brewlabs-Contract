...
contract RentEezTreasury is Ownable, ReentrancyGuard {
     ...
     */
    function initialize(IERC20 _token, address _dividendToken, address _uniRouter, address[] memory _bnbToTokenPath)
        external
        onlyOwner
    {
    ...
    */
        ...
        returns (uint256[] memory amounts)
    {
        ...
        IUniRouter02(uniRouterAddress).swapExactETHForTokensSupportingFeeOnTransferTokens{value: _amountIn}(
            amountOut.mul(slippageFactor).div(1000), _path, _to, block.timestamp.add(600)
        );
    }
    ...
    */
        ...
        returns (uint256 amountToken, uint256 amountETH, uint256 liquidity)
    {
        ...
        (amountToken, amountETH, liquidity) = IUniRouter02(uniRouterAddress).addLiquidityETH{value: _ethAmt}(
            address(_token), _tokenAmt, 0, 0, _to, block.timestamp.add(600)
        );
