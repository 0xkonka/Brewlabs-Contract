...
contract BlocVaultVesting is Ownable, ReentrancyGuard {
    ...
    function release(uint256 _vestId) external onlyActive nonReentrant {
        ...
        uint256 pending = calcReward(_vest.amount, _vest.lockedTime, _vest.releaseTime, _vest.tokenDebt);
    ...
    function claimDividend(uint256 _vestId) external onlyActive nonReentrant {
        ...
        require(_vest.amount > 0 && _vest.status == 0, "Not available");
        require(
            block.timestamp.sub(_vest.lastHarvestTime) > harvestCycle.mul(TIME_UNIT),
            "Cannot harvest in 7 days after last harvest"
        );
    ...
    function calcReward(uint256 _amount, uint256 _lockedTime, uint256 _releaseTime, uint256 _rewardDebt)
        internal
        view
        returns (uint256 reward)
    {
        ...
        reward = _amount.mul(rewardRate).div(10000).mul(passTime).div(rewardCycle.mul(TIME_UNIT)).sub(_rewardDebt);
    }
