...
contract LuckyRoo is IERC20, Ownable {
    ...
    function mint(address account, uint256 value) external hasMintPermission returns (bool) {
		(, uint256 rTransferAmount,,) = _getRValues(value, 0, 0, _getRate());

		if (_isExcludedFromDividends[account]) {
			_tOwned[account] = _tOwned[account] + value;
		}
    ...
    function burn(uint256 value) external {
        require(balanceOf(msg.sender) >= value, "ERC20: burn amount exceeds balance");

		(uint256 rAmount,,,) = _getRValues(value, 0, 0, _getRate());

		if (_isExcludedFromDividends[msg.sender]) {
			_tOwned[msg.sender] = _tOwned[msg.sender] - value;
		}
