...
contract ProjectXLockerTest is Test {
  ...
  function testAddDistribution(
    ...
    (address user, uint256 amount, uint256 lockDuration, uint256 unlockBlock, , bool claimed) = locker.distributions(
      distributor
    );
  ...
  function testRemoveDistribution(address distributor) public {
    ...
    locker.removeDistribution(distributor);
    (address user, uint256 amount, uint256 lockDuration, uint256 unlockBlock, uint256 reflectionDebt, ) = locker.distributions(distributor);
  ...
  function testPendingRelections(uint256[3] memory amounts) public {
    ...
    for(uint i = 0; i < 3; i++) {
      ...
      uint256 _accReflectionPerShare = accReflectionPerShare + reflectionAmount * (1 ether) / locker.totalAllocated();
